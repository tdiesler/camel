...
public class MllpIdleTimeoutStrategyTest extends CamelTestSupport {
    ...
    @EndpointInject(uri = "direct://sourcedefault")
    ProducerTemplate defaultStrategySource;

    @EndpointInject(uri = "direct://sourcereset")
    ProducerTemplate resetStrategySource;

    @EndpointInject(uri = "direct://sourceclose")
    ...
    protected void doPreSetup() throws Exception {
        ...
        defaultStrategyEndpoint =  Mockito.spy(
                (MllpEndpoint) mllpComponent.createEndpoint(
                        String.format("mllp://%s:%d?idleTimeout=%d", mllpServer.getListenHost(), mllpServer.getListenPort(), IDLE_TIMEOUT)));

        resetStrategyEndpoint   =  Mockito.spy(
                (MllpEndpoint) mllpComponent.createEndpoint(
                        String.format("mllp://%s:%d?idleTimeout=%d&idleTimeoutStrategy=reset", mllpServer.getListenHost(), mllpServer.getListenPort(), IDLE_TIMEOUT)));

        closeStrategyEndpoint   =  Mockito.spy(
                (MllpEndpoint) mllpComponent.createEndpoint(
                        String.format("mllp://%s:%d?idleTimeout=%d&idleTimeoutStrategy=close", mllpServer.getListenHost(), mllpServer.getListenPort(), IDLE_TIMEOUT)));
