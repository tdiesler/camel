...
| *idleTimeoutStrategy* (tcp) | decide what action to take when idle timeout occurs. Possible values are : RESET: set SO_LINGER to 0 and reset the socket CLOSE: close the socket gracefully default is RESET. | RESET | MllpIdleTimeout Strategy
...
| *camel.component.mllp.configuration.idle-timeout-strategy* | decide what action to take when idle timeout occurs. Possible values are : RESET: set SO_LINGER to 0 and reset the socket CLOSE: close the socket gracefully default is RESET. |  | MllpIdleTimeout Strategy
