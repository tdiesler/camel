...
public class SalesforceComponentIntegrationTest {
    ...
    public void shouldNotUseSSLOrTTLSVersion10() throws Exception {
        ...
        final HttpClient client = config.getHttpClient();
        ...
        final ContentExchange testGet = new ContentExchange(true);
        testGet.setURL(SALESFORCE_TLS_TEST_URL);
        testGet.setMethod(HttpMethods.GET);
        client.send(testGet);

        assertEquals(HttpExchange.STATUS_COMPLETED, testGet.waitForDone());
        assertEquals(HttpStatus.OK_200, testGet.getResponseStatus());
    }
}
