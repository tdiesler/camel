...
public class JavaSpaceConsumer extends DefaultConsumer<DefaultExchange> {
	...
	private static final transient Log LOG = LogFactory
			.getLog(JavaSpaceConsumer.class);
	...
	public JavaSpaceConsumer(final JavaSpaceEndpoint endpoint,
	...
	protected void doStart() throws Exception {
		...
		for (int i = 0; i < concurrentConsumers; ++i) {
			Task worker = new Task((JavaSpaceEndpoint) this.getEndpoint(),
					this.getProcessor(), javaSpace, transactionHelper,
					transactionTimeout, verb, templateId);
...
class Task implements Runnable {
	...
	public Task(JavaSpaceEndpoint endpoint, Processor processor,
			JavaSpace javaSpace, TransactionHelper transactionHelper,
			long transactionTimeout, int verb, String templateId)
			throws Exception {
		...
		this.verb = verb;
		if (templateId != null) {
			Entry tmpl = (Entry) this.endpoint.getCamelContext().getRegistry()
					.lookup(templateId);
	...
	public void run() {
		...
		try {
			DefaultExchange exchange = endpoint.createExchange(ExchangePattern.InOut);
