...
public class KinesisConsumer extends ScheduledBatchPollingConsumer {
    ...
    protected int poll() throws Exception {
        ...
        if (isShardClosed) {
            ...
            case ignore:
                LOG.warn("The shard related to stream {} is in closed state", getEndpoint().getStreamName());
                break;
            case silent:
                break;
            case fail:
                LOG.info("Shard Iterator reached CLOSE status:", getEndpoint().getStreamName(), getEndpoint().getShardId());
